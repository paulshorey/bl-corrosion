...
*/
require('./lib/Object.prototype.watch');
...
window.store = new Proxy(window.store, {
    set(store, propertyName, value) {
        ...
        if (typeof value !== 'object') {
            window.log(
                'error',
                `window.${propertyName} must be type "object" ... fixing to = {}`, ['before', oldValue], ['after', value]
            );
            value = {};
        }
        ...
        switch (propertyName) {
            ...
            case 'hintbox':
                ...
                if (value.opened && !value.title) {
                    window.log(
                        'error',
                        `window.${propertyName} must have property "title"` [('before', oldValue)], ['after', value]
                    );
                }
                // opened by default
                if (value.title && value.opened === undefined) {
